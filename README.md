...
- **道具優先權 1–5** — 每種道具可設定 priority；空間不足時會偏好高優先權道具
- **必用道具** — 可標記 must-use，道具互斥時優先滿足必用道具，結果區會提示是否全部放入
- **未放入提示** — 顯示所選總格數、安置率、未放入道具清單與 priority score，方便判斷取捨
...
3. **設定取捨** — priority 預設為 1，可調整到 5；勾選「必用」可要求求解器優先保留該道具
4. **開始計算** — 點擊「執行最佳化」，求解器會在時間限制內找出目前最佳擺法
5. **檢視結果** — 右側顯示可用格、填入格、所選總格數、安置率、利用率、priority score、必用狀態與未放入道具；可用箭頭切換候選解法
6. **切換語言** — 點擊右上角語言按鈕
...
- 候選解排序依序考慮：must-use 完成度、priority score、填入格數、未放入數量與穩定 signature
