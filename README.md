...
輸入棋盤可用區域、道具數量與可選的必用設定，求解器會在背景執行緒的時間限制內計算較佳的擺放方案。
...
- **少搬動偏好** — 保留上一個成功結果作為基準；同等最佳解之間，優先保留更多完全相同位置與旋轉的道具
- **背景求解** — 最佳化在 Web Worker 中執行，避免較長搜尋阻塞主 UI 操作
...
5. **開始計算** — 點擊「執行最佳化」，求解器會在時間限制內找出目前最佳擺法；已有上一個成功結果時，會在不犧牲原本最佳目標的前提下偏好少搬動方案
...
├── solverWorker.ts         # Web Worker 入口，讓求解在背景執行緒進行
...
├── App.tsx                 # 主元件：三欄式 UI、求解狀態與 Worker 管理
├── main.tsx                # React 進入點
└── styles.css              # 全域樣式
```
...
- 最佳方案排序依序考慮：必用道具完成度、填入格數、未放入數量、保留上一解的完全相同 placement 數量，以及穩定 signature；因此 UI 只顯示排序後的最佳方案
- 少搬動偏好只在主目標完全相同時生效，不會為了保留舊位置而犧牲必用、填入格數或未放入數量
- 當所選道具總格數小於等於可用格時，目標填入格為所選總格數；超過可用格時，目標填入格為可用格
- 求解器保留時間上限：第一次或沒有上一解時 UI 使用 1 秒；有上一個成功結果可作為少搬動基準時，UI 使用 3 秒以比較更多同等最佳解
- UI 透過 Web Worker 執行求解，較長搜尋不會阻塞主執行緒；極端案例仍可能回傳時間內最佳解。只有 `provenOptimal` 為 `true` 時，才代表搜尋已完整完成並證明目前排序目標下的最佳解
