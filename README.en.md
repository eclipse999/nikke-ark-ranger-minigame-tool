...
- **9×9 Board** — Click cells to toggle available / unavailable, with reset and fill-all shortcuts
- **P01–P15 Complete Items** — All 15 item types, auto-generating 0°/90°/180°/270° rotated shapes with deduplication
- **Semi-Automatic Screenshot Import** — Import a full game screenshot; the browser locally estimates the 9×9 board state for manual correction
...
1. **Set up the board** — Click gray cells to mark them unavailable (dark), or use "Fill All" / "Reset" buttons
...
Screenshot import is a semi-automatic helper that only processes images locally in your browser — nothing is uploaded. After importing, double-check the board state and correct manually if needed.
...
├── screenshotImport.ts # Semi-automatic screenshot import and 9×9 grid detection
